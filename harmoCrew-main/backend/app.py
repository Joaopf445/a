...
import jwt
import datetime # Mantenha este import
from functools import wraps
from werkzeug.security import generate_password_hash, check_password_hash
from db.init_db import init_db

# Inicializa o DB (garante que as tabelas existam)
init_db()
...
# Configuração CORS explícita para permitir requisições da origem do seu frontend
# e os headers necessários, além de permitir o método OPTIONS para preflight.
CORS(app, 
     resources={r"/*": {"origins": ["http://localhost:3000", "http://127.0.0.1:3000"]}}, # Permite ambas as formas de localhost
...
def token_required(f): #
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            # Use datetime.UTC para timezones explícitos
...
def register():
    ...
    default_profile_pic_url = f"https://i.pravatar.cc/150?u={email}" # Fallback de imagem de perfil
...
def login():
    ...
    try:
        ...
        cursor.execute("SELECT id, nome, email, senha, profile_pic_url FROM users WHERE email = %s", (email,)) # Adicionado profile_pic_url
        user = cursor.fetchone()

        if not user or not check_password_hash(user['senha'], senha):
            return jsonify({"message": "Email ou senha incorretos."}), 401

        # Usar datetime.UTC para timezone-aware datetime objects
        expiration_time = datetime.datetime.now(datetime.UTC) + datetime.timedelta(hours=24)
        token = jwt.encode({
            'email': user['email'],
            'id': user['id'], # Adicionando ID ao token
            'exp': expiration_time
...
def get_posts(current_user_email):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        # Inclui os novos campos titulo, audio_url e profile_pic_url do usuário
        cursor.execute("""
        ...
        for post_item in posts: # Renomeado para post_item para evitar conflito
            ...
            # audio_url já virá como está no banco (pode ser NULL)

        return jsonify({"posts": posts})
    except mysql.connector.Error as err:
        print(f"Erro em GET /posts: {err}")
...
def create_post(current_user_email):
    ...
    # Correção para AttributeError com audio_url
    raw_audio_url = data.get("audio_url")
    if isinstance(raw_audio_url, str):
        audio_url = raw_audio_url.strip() or None
    else:
        audio_url = None

    if not titulo:
        return jsonify({"message": "O título do post não pode ser vazio."}), 400
    if not texto: # Descrição também é importante
    ...
    try:
        ...
        # Retorna o post recém-criado com todos os campos
        # Usar datetime.UTC para timezone-aware datetime objects
        ...
        new_post_data = {
            ...
            "texto": texto,
            "audio_url": audio_url,
            "created_at": created_at_datetime.strftime("%Y-%m-%d %H:%M:%S"), # Data formatada
            "data": created_at_datetime.strftime("%Y-%m-%d %H:%M:%S") # Campo 'data' para consistência
        }
        return jsonify({"message": "Post criado com sucesso!", "post": new_post_data}), 201
    except mysql.connector.Error as err:
        print(f"Erro em POST /posts: {err}")
        return jsonify({"message": "Erro no banco de dados ao criar post.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()

# Endpoint para "Você Segue" (usuário logado)
...
def get_my_following(current_user_email):
    ...
    try:
        ...
        print(f"Erro em /api/me/following: {err}")
        return jsonify({"message": "Erro no banco de dados ao buscar quem você segue.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()
...
def candidatar(current_user_email, post_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor(dictionary=True) # Usar dictionary=True consistentemente
        cursor.execute("SELECT id FROM users WHERE email = %s", (current_user_email,))
        user = cursor.fetchone()
        if not user:
            return jsonify({"message": "Usuário não encontrado."}), 404
        ...
        print(f"Erro em /candidatar: {err}")
        return jsonify({"message": "Erro no banco de dados ao candidatar.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()
...
def candidaturas_recebidas(current_user_email):
    ...
    try:
        ...
        SELECT 
            ...
            p.titulo AS titulo_post, p.texto AS texto_post, 
        ...
        print(f"Erro em /candidaturas_recebidas: {err}")
        return jsonify({"message": "Erro no banco de dados ao buscar candidaturas recebidas.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()
...
@token_required
def atualizar_candidatura_status(current_user_email, candidatura_id, acao): # Unificada com a rota anterior
    ...
    try:
        ...
        print(f"Erro em /candidaturas/acao: {err}")
        return jsonify({"message": "Erro no banco de dados ao atualizar candidatura.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()
...
@token_required # Mantido token_required, mas pode ser removido se a busca for pública
def search_users(current_user_email):
    ...
    try:
        ...
        print(f"Erro em /search_users: {err}")
        return jsonify({"message": "Erro no banco de dados ao buscar usuários.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()


@app.route("/user/<int:view_user_id>", methods=["GET"]) # Alterado para view_user_id para clareza
@token_required
def get_user_by_id(current_user_email, view_user_id): #
    ...
    try:
        ...
        if not logged_user: # Checagem extra
        ...
        print(f"Erro em /user/id: {err}")
        return jsonify({"message": "Erro no banco de dados ao buscar usuário.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()


@app.route("/follow/<int:user_to_follow_id>", methods=["POST"]) # Alterado para user_to_follow_id
@token_required
    ...
    try:
        ...
        try:
            ...
            if e.errno == 1062: # Código de erro para entrada duplicada
                ...
                raise e # Re-levanta outros IntegrityErrors
        
        return jsonify({"message": "Seguindo usuário com sucesso!"})
    except mysql.connector.Error as err:
        print(f"Erro em /follow: {err}")
        return jsonify({"message": "Erro no banco de dados ao seguir usuário.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()


@app.route('/unfollow/<int:user_to_unfollow_id>', methods=['POST']) # Alterado para user_to_unfollow_id
@token_required
def unfollow(current_user_email, user_to_unfollow_id): #
    ...
    try:
        ...
        cursor.execute("SELECT 1 FROM users WHERE id = %s", (user_to_unfollow_id,))
        if not cursor.fetchone():
            return jsonify({"message": "Usuário a deixar de seguir não encontrado."}), 404

        cursor.execute("SELECT 1 FROM seguidores WHERE seguidor_id = %s AND seguido_id = %s", (current_user_id, user_to_unfollow_id))
        if not cursor.fetchone():
        ...
        if cursor.rowcount == 0:
             return jsonify({"message": "Não foi possível deixar de seguir. Usuário não era seguido."}), 400

        return jsonify({"message": "Deixou de seguir o usuário com sucesso!"}), 200
    except mysql.connector.Error as err:
        print(f"Erro em /unfollow: {err}")
        return jsonify({"message": "Erro no banco de dados ao deixar de seguir o usuário.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()


@app.route("/followers/<int:user_id_profile>", methods=["GET"]) # Alterado para user_id_profile
@token_required
def get_followers(current_user_email, user_id_profile): #
    ...
    try:
        ...
        print(f"Erro em /followers: {err}")
        return jsonify({"message": "Erro ao buscar seguidores.", "error": str(err)}), 500
    finally:
        if cursor: cursor.close()
        if conn and conn.is_connected(): conn.close()

# Rota para buscar quem um usuário específico (user_id_profile) está seguindo
@app.route("/following/<int:user_id_profile>", methods=["GET"])
@token_required
def get_user_following(current_user_email, user_id_profile):
    conn = None
    cursor = None
    try:
        conn = get_connection()
        ...
        cursor.execute("""
            ...
            JOIN users u ON s.seguido_id = u.id
            WHERE s.seguidor_id = %s 
        """, (user_id_profile,)) # Aqui usamos user_id_profile como seguidor_id
        ...
        print(f"Erro em /following/id: {err}")
